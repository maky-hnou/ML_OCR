...
def train_shadownet(dataset_dir, weights_path=None):
    ...
    (images, labels, imagenames) = \
        decoder.read_features(ops.join(dataset_dir,
                              'train_feature.tfrecords'), num_epochs=None)

    (inputdata, input_labels, input_imagenames) = \
        tf.train.shuffle_batch(tensors=[images, labels, imagenames],
                               batch_size=32, capacity=1000 + 2 * 32,
                               min_after_dequeue=100, num_threads=1)

    inputdata = tf.cast(x=inputdata, dtype=tf.float32)

    # initializa the net model
    shadownet = ShadowNet(phase='Train', hidden_nums=256,
    ...
    cost = \
        tf.reduce_mean(tf.nn.ctc_loss(labels=input_labels, inputs=net_out,
    ...
    sequence_dist = tf.reduce_mean(tf.edit_distance(tf.cast(decoded[0],
                                                    tf.int32), input_labels))
        ...
        for epoch in range(train_epochs):
            ...
            if (epoch % config.cfg.TRAIN.DISPLAY_STEP == 0):
                logger.info('Epoch: {:d} cost= {:9f} seq distance= {:9f} train'
                            ' accuracy= {:9f}'.format(epoch + 1, c,
                                                      seq_distance, accuracy))
